...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    public final static Long DEFAULT_TIMESTAMP = 1000L;
    ...
    public static String getStartTimeToday() {
        return DateUtils.convertDateToStr(BASE_START_DAY + ((System.currentTimeMillis() - BASE_START_DAY) / ONE_DAY_MILLIS)
    ...
    public static String getStartTimeYesterday() {
        return DateUtils.convertDateToStr(BASE_START_DAY
                + ((System.currentTimeMillis() - BASE_START_DAY) / ONE_DAY_MILLIS - 1)
                * ONE_DAY_MILLIS, DEFAULT_LONG_DATE_FORMAT);
    }
    ...
    public static String getStartTimeWeekDay() {
        return DateUtils.convertDateToStr(BASE_START_DAY + ((System.currentTimeMillis() - BASE_START_DAY) / ONE_DAY_MILLIS - 7)
                * ONE_DAY_MILLIS, DEFAULT_LONG_DATE_FORMAT);
    }
    ...
    public static String getLastDayOfMonth(String monthTime) {
        ...
        Date endTime = add(convertStrToDate(monthTime, DEFAULT_SPLIT_PAY_FORMAT), DateUtils.TimeUnit.ss, -1);
    ...
    public static long convertTimeToStamp(String time) {
        ...
        builder.append(DateUtils.format(new Date()));
        builder.append(" ");
        builder.append(time);
        if (time.length() == 5) {
        ...
        return DateUtils.parseLong(builder.toString()).getTime();
    }
    ...
    public static String getBeforeMonth(Date date) {
        SimpleDateFormat format = new SimpleDateFormat(DEFAULT_MONTH_DATE_FORMAT);
     ...
     * @return 毫秒
     */
    public static long getStartTimeOfNowAndMinusMonths(int monthsToSubtract) {
        return LocalDateTime.of(LocalDate.now().minusMonths(monthsToSubtract), LocalTime.MIN).toInstant(ZoneOffset.of("+8")).toEpochMilli();
    }
    /**
     * 获取当前时间减去指定天后当天起始时间
     * @param daysToSubtract 减去的天
     * @return 毫秒
     */
    public static long getStartTimeOfNowAndMinusDay(int daysToSubtract) {
        return LocalDateTime.of(LocalDate.now().minusDays(daysToSubtract), LocalTime.MIN).toInstant(ZoneOffset.of("+8")).toEpochMilli();
    }
